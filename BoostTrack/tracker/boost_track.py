...
class BoostTrack(object):
    def __init__(self, video_name: Optional[str] = None):
        ...
        if GeneralSettings['use_embedding']:
            self.embedder = EmbeddingComputer(GeneralSettings['dataset'], GeneralSettings['test_dataset'], True, GeneralSettings['reid_type'], GeneralSettings['num_classes'], GeneralSettings['reid_path'])
        else:
            self.embedder = None
